<?php
/**
 * MODULE ADMIN — Pages de Capture  v1.3
 * /admin/modules/content/pages-capture/index.php
 * v1.3 : vue liste (défaut) + vue grille, toggle persisté sessionStorage
 */
...
/* Banner */
.cap-banner { background:var(--surface,#fff); border-radius:16px; padding:24px 28px; margin-bottom:20px; display:flex; align-items:center; justify-content:space-between; border:1px solid var(--border,#e5e7eb); position:relative; overflow:hidden; }
.cap-banner::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; background:linear-gradient(90deg,#d97706,#f59e0b,#fbbf24); }
.cap-banner-left h2 { font-size:1.3rem; font-weight:700; color:var(--text,#111827); margin:0 0 4px; display:flex; align-items:center; gap:10px; }
.cap-banner-left p { color:var(--text-2,#6b7280); font-size:.83rem; margin:0; }
.cap-stats { display:flex; gap:8px; flex-wrap:wrap; }
.cap-stat { text-align:center; padding:10px 14px; background:var(--surface-2,#f9fafb); border-radius:12px; border:1px solid var(--border,#e5e7eb); min-width:68px; transition:all .2s; }
.cap-stat:hover { border-color:#d97706; }
.cap-stat .num { font-size:1.4rem; font-weight:800; line-height:1; }
...
/* Toolbar */
.cap-toolbar { display:flex; align-items:center; justify-content:space-between; margin-bottom:14px; flex-wrap:wrap; gap:10px; }
.cap-toolbar-l { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
.cap-toolbar-r { display:flex; align-items:center; gap:8px; }

/* Filtres status */
.cap-filters { display:flex; gap:3px; background:var(--surface,#fff); border:1px solid var(--border,#e5e7eb); border-radius:10px; padding:3px; }
.cap-fbtn { padding:6px 14px; border:none; background:transparent; color:var(--text-2,#6b7280); font-size:.77rem; font-weight:600; border-radius:6px; cursor:pointer; transition:all .15s; display:flex; align-items:center; gap:5px; text-decoration:none; font-family:inherit; }
.cap-fbtn:hover { background:var(--surface-2,#f9fafb); color:var(--text,#111827); }
.cap-fbtn.active { background:#d97706; color:#fff; }
.cap-fbtn .badge { font-size:.65rem; padding:1px 6px; border-radius:10px; background:var(--surface-2,#f3f4f6); font-weight:700; color:var(--text-3,#9ca3af); }
.cap-fbtn.active .badge { background:rgba(255,255,255,.22); color:#fff; }

/* Persona pills */
.cap-persona-filter { display:flex; gap:5px; flex-wrap:wrap; }
...
.cap-search input { padding:7px 11px 7px 32px; background:var(--surface,#fff); border:1px solid var(--border,#e5e7eb); border-radius:10px; color:var(--text,#111827); font-size:.81rem; width:190px; font-family:inherit; transition:all .2s; }
.cap-search input:focus { outline:none; border-color:#d97706; width:220px; }
.cap-search i { position:absolute; left:10px; top:50%; transform:translateY(-50%); color:var(--text-3,#9ca3af); font-size:.74rem; }
...
/* Btn principal */
.cap-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 16px; border-radius:10px; font-size:.81rem; font-weight:600; cursor:pointer; border:none; transition:all .15s; font-family:inherit; text-decoration:none; }
.cap-btn-primary { background:#d97706; color:#fff; box-shadow:0 1px 4px rgba(217,119,6,.25); }
.cap-btn-primary:hover { background:#b45309; color:#fff; }
...
.cap-table thead th { padding:10px 13px; font-size:.63rem; font-weight:700; text-transform:uppercase; letter-spacing:.05em; color:var(--text-3,#9ca3af); background:var(--surface-2,#f9fafb); border-bottom:1px solid var(--border,#e5e7eb); text-align:left; white-space:nowrap; }
...
.cap-table td { padding:11px 13px; font-size:.82rem; color:var(--text,#111827); vertical-align:middle; }
.cap-table td.center { text-align:center; }
.cap-title-cell a { font-weight:600; color:var(--text,#111827); text-decoration:none; transition:color .15s; }
.cap-title-cell a:hover { color:#d97706; }
.cap-slug { font-family:monospace; font-size:.7rem; color:var(--text-3,#9ca3af); margin-top:2px; }
.cap-desc { font-size:.7rem; color:var(--text-3,#9ca3af); margin-top:1px; max-width:240px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.cap-type-badge { display:inline-block; font-size:.6rem; font-weight:700; padding:1px 7px; border-radius:8px; text-transform:uppercase; letter-spacing:.04em; }
...
.cap-res-icon { font-size:16px; width:28px; height:28px; border-radius:7px; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
.cap-res-name { font-size:.73rem; font-weight:700; color:var(--text,#111827); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:150px; }
...
.cap-metric { display:flex; flex-direction:column; align-items:center; gap:2px; }
.cap-metric-val { font-size:.83rem; font-weight:700; }
.cap-metric-val.blue   { color:#3b82f6; }
.cap-metric-val.green  { color:#10b981; }
.cap-metric-val.none   { color:var(--text-3,#9ca3af); }
.cap-metric-lbl { font-size:.58rem; color:var(--text-3,#9ca3af); }
.cap-taux-wrap { display:flex; flex-direction:column; align-items:center; gap:3px; min-width:54px; }
.cap-taux-ring { width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:.64rem; font-weight:800; border:3px solid transparent; }
.cap-taux-ring.excellent { background:#ecfdf5; border-color:#10b981; color:#059669; }
.cap-taux-ring.good      { background:#eff6ff; border-color:#3b82f6; color:#2563eb; }
.cap-taux-ring.ok        { background:#fefce8; border-color:#f59e0b; color:#d97706; }
.cap-taux-ring.bad       { background:#fef2f2; border-color:#ef4444; color:#dc2626; }
.cap-taux-ring.none      { background:var(--surface-2,#f9fafb); border-color:var(--border,#e5e7eb); border-style:dashed; color:var(--text-3,#9ca3af); }
.cap-taux-bar { width:32px; height:3px; background:var(--border,#e5e7eb); border-radius:2px; overflow:hidden; }
.cap-taux-fill { height:100%; border-radius:2px; }
.cap-taux-fill.excellent { background:#10b981; }
.cap-taux-fill.good      { background:#3b82f6; }
.cap-taux-fill.ok        { background:#f59e0b; }
.cap-taux-fill.bad       { background:#ef4444; }
.cap-actions { display:flex; gap:3px; justify-content:flex-end; }
.cap-actions a,.cap-actions button { width:28px; height:28px; border-radius:8px; display:flex; align-items:center; justify-content:center; color:var(--text-3,#9ca3af); background:transparent; border:1px solid transparent; cursor:pointer; transition:all .12s; text-decoration:none; font-size:.76rem; }
...
.cap-date { font-size:.71rem; color:var(--text-3,#9ca3af); white-space:nowrap; }

/* ══ VUE GRILLE ══════════════════════════════════════════════════ */
.cap-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:14px; }
.cap-card {
    background:var(--surface,#fff); border-radius:14px; border:1px solid var(--border,#e5e7eb);
...
.cap-card-res-icon { font-size:16px; width:26px; height:26px; border-radius:6px; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
...
/* Pagination */
.cap-pagination { display:flex; align-items:center; justify-content:space-between; padding:13px 16px; border-top:1px solid var(--border,#e5e7eb); font-size:.77rem; color:var(--text-3,#9ca3af); }
.cap-pagination a { padding:5px 11px; border:1px solid var(--border,#e5e7eb); border-radius:9px; color:var(--text-2,#6b7280); text-decoration:none; font-weight:600; transition:all .15s; }
...
/* Flash / Empty */
.cap-flash { padding:11px 16px; border-radius:10px; font-size:.83rem; font-weight:600; margin-bottom:14px; display:flex; align-items:center; gap:8px; }
...
@media(max-width:960px){
    .cap-banner { flex-direction:column; gap:14px; align-items:flex-start; }
...
<div class="cap-banner">
    <div class="cap-banner-left">
        <h2><i class="fas fa-bolt" style="color:#d97706"></i> Pages de Capture</h2>
    ...
    <div class="cap-stats">
        ...
        <div class="cap-stat"><div class="num green"><?= round($stats['total_leads']/$stats['total_vues']*100,1) ?>%</div><div class="lbl">Taux conv.</div></div>
        <?php endif; ?>
    </div>
</div>
...
<div class="cap-toolbar">
    <div class="cap-toolbar-l">
        <div class="cap-filters">
            <?php
            $filters = ['all'=>['label'=>'Toutes','count'=>$stats['total']],'active'=>['label'=>'Actives','count'=>$stats['active']],'draft'=>['label'=>'Inactives','count'=>$stats['draft']]];
            ...
            <a href="<?= $url ?>" class="cap-fbtn<?= $isA?' active':'' ?>">
                <?= $f['label'] ?> <span class="badge"><?= (int)$f['count'] ?></span>
            </a>
            <?php endforeach; ?>
        </div>

        <?php if ($hasRessources): ?>
        <div class="cap-persona-filter">
            <?php
            $pfAll = '?page=captures';
            ...
            <a href="<?= $pfAll ?>" class="cap-pf-btn <?= $filterPersona==='all'?'active':'' ?>"
               style="<?= $filterPersona==='all'?'background:#64748b;border-color:#64748b;':'' ?>">Tous</a>
            ...
            ?>
            <a href="<?= $pfUrl ?>" class="cap-pf-btn <?= $isP?'active':'' ?>"
               style="<?= $isP?"background:{$pl['color']};border-color:{$pl['color']};":'' ?>"><?= $pl['label'] ?></a>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="cap-toolbar-r">
        ...
        <div class="cap-view-toggle">
            <button class="cap-view-btn active" id="btnList" onclick="CAP.setView('list')" title="Vue liste">
                <i class="fas fa-list"></i>
            </button>
            <button class="cap-view-btn" id="btnGrid" onclick="CAP.setView('grid')" title="Vue grille">
...
<div class="cap-empty">
    ...
    <p><?= ($searchQuery || $filterPersona !== 'all') ? 'Aucun résultat. <a href="?page=captures" style="color:#d97706">Effacer</a>' : 'Créez votre première page de capture.' ?></p>
</div>
<?php else: ?>
        ...
        <table class="cap-table">
            <thead>
                <tr>
                    ...
                    <th>Statut</th>
                    <th class="center">Vues</th>
                    <th class="center">Leads</th>
                    <th class="center">Taux conv.</th>
                    <th>Dernière conv.</th>
                    <th>Créée le</th>
            ...
            <tr data-id="<?= (int)$cap['id'] ?>">
                ...
                <td class="center"><div class="cap-metric"><span class="cap-metric-val blue"><?= $vues>0?number_format($vues,0,',',' '):'—' ?></span><?php if($vues>0):?><span class="cap-metric-lbl">vues</span><?php endif;?></div></td>
                <td class="center"><div class="cap-metric"><span class="cap-metric-val green"><?= $leads>0?$leads:'—' ?></span><?php if($leads>0):?><span class="cap-metric-lbl">leads</span><?php endif;?></div></td>
                <td class="center">
                    <div class="cap-taux-wrap">
                        <div class="cap-taux-ring <?= $tCls ?>"><?= $taux>0?$taux.'%':'—' ?></div>
                        <div class="cap-taux-bar"><div class="cap-taux-fill <?= $tCls ?>" style="width:<?= min(100,$taux*2) ?>%"></div></div>
                    </div>
                </td>
                <td><span class="cap-date"><?= $dL ?></span></td>
...
<div id="capModal" style="display:none;position:fixed;inset:0;z-index:9999;align-items:center;justify-content:center;">
    ...
    ">
        ...
        <div style="display:flex;gap:8px;justify-content:flex-end;padding:12px 20px 18px;border-top:1px solid #f3f4f6;">
            <button onclick="CAP.modalClose()" id="capModalCancel" style="
                padding:9px 20px;border-radius:10px;border:1px solid #e5e7eb;
                background:#fff;color:#374151;font-size:.83rem;font-weight:600;
                cursor:pointer;transition:all .15s;font-family:inherit;
            " onmouseover="this.style.borderColor='#d97706';this.style.color='#d97706'" onmouseout="this.style.borderColor='#e5e7eb';this.style.color='#374151'">
                Annuler
            </button>
            <button id="capModalConfirm" style="
...
const CAP = {
    ...
    modal({ icon, iconBg, iconColor, title, msg, confirmLabel, confirmColor, onConfirm }) {
        ...
        const iw    = document.getElementById('capModalIconWrap');
        const hdr   = document.getElementById('capModalHeader');
        const tEl   = document.getElementById('capModalTitle');
        const mEl   = document.getElementById('capModalMsg');
        const btnOk = document.getElementById('capModalConfirm');

        iw.innerHTML  = icon;
        iw.style.background   = iconBg;
        iw.style.color        = iconColor;
        hdr.style.background  = iconBg + '33';
