...
class GmbScraperController
{
    ...
    public function addToList(int $contactId, int $listId): bool
    {
        try {
            ...
            // Update count
            $updateStmt = $this->db->prepare("UPDATE gmb_contact_lists SET contacts_count = (SELECT COUNT(*) FROM gmb_contact_list_members WHERE list_id = ?) WHERE id = ?");
            $updateStmt->execute([$listId, $listId]);
            return true;
        } catch (PDOException $e) {
            return false;
        }
