 ...
 * v3 : + routes biens immobiliers (/biens-immobiliers, /biens/{slug})
 * v3.1 : renderHeader supporte nav_align (left/center/right)
 */
...
if (!function_exists('renderHeader')) {
    function renderHeader(?array $header): string {
        ...
        // Custom HTML override (si renseigné et pas du JSON-menu)
        ...
        $shadow  = !empty($header['shadow'])
                    ? 'box-shadow:0 2px 12px rgba(0,0,0,.10);'
                    : '';
        $border  = !empty($header['border_bottom'])
                    ? "border-bottom:2px solid $hv;"
                    : 'border-bottom:1px solid #e2d9ce;';
        $bp      = (int)($header['mobile_breakpoint'] ?? 768);

        // ── Alignement nav (nouveau champ v2.4) ──────────────
        ...
        // ════════════════════════════════════════════════════
        // Layout desktop selon nav_align
        // ════════════════════════════════════════════════════
        switch ($navAlign) {

            // ── GAUCHE : Logo | Nav collée gauche | CTA droite ──
            case 'left':
                $desktopLayout = '
    <div style="max-width:1260px;margin:0 auto;padding:0 24px;
                display:grid;grid-template-columns:auto 1fr auto;
                align-items:center;height:'.$ht.'px;gap:24px;position:relative">

      '.$logoHtml.'

      <nav class="fh-desktop" style="display:flex;align-items:center;justify-self:start">
        '.$navLinks.'
      </nav>
      ...
      <!-- Burger mobile -->
      <button class="fh-burger" onclick="this.closest(\'.'.$uid.'\').querySelector(\'.fh-mobile-nav\').classList.toggle(\'open\')"
        ...
        <span style="width:22px;height:2px;background:'.$tc.';display:block;border-radius:2px"></span>
            ...
            // ── DROITE : Logo | [Nav + CTA groupés à droite] ────
            case 'right':
                $desktopLayout = '
    <div style="max-width:1260px;margin:0 auto;padding:0 24px;
      ...
      <button class="fh-burger" onclick="this.closest(\'.'.$uid.'\').querySelector(\'.fh-mobile-nav\').classList.toggle(\'open\')"
        ...
        <span style="width:22px;height:2px;background:'.$tc.';display:block;border-radius:2px"></span>
            ...
            // ── CENTRE (défaut) : Logo | Nav centrée | CTA droite ──
            case 'center':
            default:
                $desktopLayout = '
    <div style="max-width:1260px;margin:0 auto;padding:0 24px;
                display:grid;grid-template-columns:1fr auto 1fr;
                align-items:center;height:'.$ht.'px;gap:16px;position:relative">

      '.$logoHtml.'

      <nav class="fh-desktop" style="display:flex;align-items:center;justify-self:center">
        '.$navLinks.'
      </nav>

      <div class="fh-desktop fh-right" style="display:flex;align-items:center;gap:16px;flex-shrink:0;justify-self:end">
        '.$phoneHtml.'
        '.$ctaHtml.'
      </div>
      ...
      <!-- Burger mobile -->
      <button class="fh-burger" onclick="this.closest(\'.'.$uid.'\').querySelector(\'.fh-mobile-nav\').classList.toggle(\'open\')"
        style="display:none;flex-direction:column;gap:5px;background:none;border:none;cursor:pointer;padding:6px;flex-shrink:0;justify-self:end">
        <span style="width:22px;height:2px;background:'.$tc.';display:block;border-radius:2px"></span>
        <span style="width:22px;height:2px;background:'.$tc.';display:block;border-radius:2px"></span>
        <span style="width:22px;height:2px;background:'.$tc.';display:block;border-radius:2px"></span>
      </button>
        ...
        // ── CSS responsive (identique pour les 3 layouts) ────
        $responsiveCss = '
<style>
...
@media (max-width:'.$bp.'px) {
  ...
  .'.$uid.' .fh-mobile-nav {
    ...
    border-top:1px solid #e2d9ce;z-index:999
  }
...
@media (min-width:'.($bp+1).'px) {
  ...
  .'.$uid.' .fh-mobile-nav { display:none !important }
        ...
        // ── Menu mobile (commun aux 3 layouts) ───────────────
        $mobileNav = '
...
if (!function_exists('renderFooter')) {
    function renderFooter(?array $footer): string {
        ...
        $colsSection = $colsHtml
            ...
            : '<div style="margin-bottom:24px">'.$contactHtml.'</div>';
...
if ($parts[0] === 'biens' && !empty($parts[1])) {
    ...
    $propFound = false;
    try {
        foreach (['statut','status'] as $col) {
            try {
                $stProp = $db->prepare("SELECT id FROM properties WHERE slug = ? AND `{$col}` IN ('actif','active','disponible') LIMIT 1");
                $stProp->execute([$slug]);
                if ($stProp->fetchColumn()) { $propFound = true; break; }
